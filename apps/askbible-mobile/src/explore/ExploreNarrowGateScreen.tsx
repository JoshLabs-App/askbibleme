...
function parseRef(raw: string): ParsedRef | null {
  ...
  const bookId = getExploreModulesContent().narrowGate.bookAbbrToId[abbr];
...
export function ExploreNarrowGateScreen() {
  ...
  const narrowGateCategories: ExploreModulesCategory[] = modules.narrowGate.categories;
  const narrowGateTitlesEn = modules.narrowGate.titlesEn;
  ...
  const categoryTitles = locale === "en" ? narrowGateTitlesEn : null;

  const parsedRefByRaw = useMemo(() => {
    const entries = narrowGateCategories.flatMap((category) => category.refs).map((raw) => [
      raw,
      parseRef(raw),
    ] as const);
    return Object.fromEntries(entries) as Record<string, ParsedRef | null>;
  }, [narrowGateCategories]);
  ...
  useEffect(() => {
    ...
    const category = narrowGateCategories[expandedCategoryIndex];
    if (!category) return;
    ...
    <View style={shared.root}>
      ...
      >
        ...
        <View style={styles.accordionList}>
          {narrowGateCategories.map((category, index) => (
