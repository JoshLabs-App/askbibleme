...
> 本文是**代码地图**，不是产品哲学文档。产品边界 / 语气 / 工作方式见根目录 `AGENTS.md` 与 `docs/HANDOFF.md`，改代码前仍应先读那两份。这里只回答一个问题：**改某个功能该先看哪些文件、模块之间怎么连**。
>
...
- 真机装「独立版」（无 DEV 横幅、不走 TestFlight）不能直接用 Release 配置装机（Apple 禁止 App Store 描述文件直装）；也不用折腾 EAS ad-hoc（这个项目里从没配成功过）。做法和已知的 Expo CLI devicectl 卡死坑点见 `docs/HANDOFF.md` 「真机装独立版」一节。
