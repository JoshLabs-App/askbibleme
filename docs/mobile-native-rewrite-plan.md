...
#### 5. 欢迎页缺「每日读经提醒」
- 现状：RN 的首次欢迎页有语言 + 闹钟 + 登录三段，原生只做了语言 + 登录。
- 原因：原生两端都还没有本地通知（提醒）功能。
- 需要你决定：要不要做本地通知（每日读经提醒、金句推送），这是一块独立的功能。

#### 6. 抽屉里没有「删除账户」
- 现状：RN 的用户菜单有「删除账户」，原生没有，因为原生的会员模块还没接删除账户接口。
- 需要你决定：要不要补。涉及不可逆操作，我不会自己加。

#### 7. 「划重点」没接
- 现状：RN 的长按操作单里有「划重点」（给经文里的词上色），原生没有。
- 需要你决定：要不要补。
