...
type EncouragingSection = {
  titleZh: string;
  titleEn: string;
  quotes: EncouragingQuote[];
};

const ENCOURAGING_WORDS_BOTTOM_PAD = 140;

const ENCOURAGING_WORDS_SECTIONS: EncouragingSection[] = [
  {
    titleZh: "安静 / 平安 / 放松",
...
export function ExploreEncouragingWordsScreen() {
      ...
      <ParchmentBottomFadeScrollView
        ...
        contentContainerStyle={[
          shared.scroll,
          {
            ...
            paddingBottom: ENCOURAGING_WORDS_BOTTOM_PAD + insets.bottom,
          },
        ]}
      >
        <Pressable onPress={() => router.back()} style={shared.backLink} accessibilityRole="button">
          <Text style={shared.backLinkText}>{t("pages.explore.encouragingWordsBack")}</Text>
        </Pressable>

        <Text style={styles.pageTitle}>{t("pages.explore.encouragingWordsTitle")}</Text>
        <Text style={styles.eyebrow}>{t("pages.explore.encouragingWordsSubtitle")}</Text>

        <View style={styles.sectionList}>
          {ENCOURAGING_WORDS_SECTIONS.map((section) => (
            <View key={section.titleEn} style={styles.sectionCard}>
              <Text style={styles.sectionTitle}>{sectionTitle(section, locale)}</Text>
              <View style={styles.quoteList}>
                {section.quotes.map((quote) => (
                  <View key={quote.id} style={styles.quoteItem}>
                    <Text style={styles.quoteEn}>
                      {quote.id}. {quote.en}
                    </Text>
                    <Text style={styles.quoteZh}>{quote.zh}</Text>
                    {quote.ref ? <Text style={styles.quoteRef}>— {quote.ref}</Text> : null}
                  </View>
                ))}
              </View>
            </View>
