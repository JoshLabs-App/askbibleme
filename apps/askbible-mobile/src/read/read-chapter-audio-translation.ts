...
function pickLanguageMatchedAudioTranslation(primaryId: string, catalog: BibleTranslationMeta[]): string | null {
  ...
  if (lang === "en") {
    return firstSupportedByIds(["web-en", "bbe-en"], catalog);
  }
  if (lang === "es") {
    return firstSupportedByIds(["blm-es"], catalog);
  }
  if (lang === "zh") {
    return firstSupportedByIds(["cuv-simp", "cuv-trad"], catalog);
  }
  return null;
}
...
export function resolveChapterAudioTranslationId(
  ...
  return firstSupportedByIds(["cuv-simp", "web-en", "bbe-en", "blm-es"], index?.translations ?? [])
    ?? "cuv-simp";
}
