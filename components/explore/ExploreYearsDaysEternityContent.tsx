...
export function ExploreYearsDaysEternityContent({ enScriptureBodyByRef, enRefLabelByRaw }: Props) {
  ...
  return (
    <div className="mx-auto w-full max-w-xl flex-1 px-5 pb-24 pt-6 text-ink sm:max-w-2xl md:px-8">
      ...
      >
        {t("pages.explore.yearsDaysEternityBack")}
      </Link>

      <header className="mt-4 text-center">
        <h1 className="font-serif text-[clamp(1.35rem,4vw,1.75rem)] font-medium leading-snug text-ink/92">
          {doc.pageTitle}
        </h1>
      ...
      <section className="mt-8 space-y-4 text-center text-[15px] leading-relaxed text-ink/72">
      ...
      <section className="mt-10 space-y-4 text-center">
        {filterEternityProse(doc.intro).map((lines, i) => (
          <div key={`intro-${i}`} className="space-y-1 text-[15px] leading-relaxed text-ink/72">
            {lines.map((line, j) => (
              <p key={j}>{line}</p>
            ))}
          </div>
        ...
        <div className="mt-4 text-left">
          <ScriptureFlow scriptures={filterEternityScriptures(doc.intro)} />
        </div>
      </section>
    </div>
  );
