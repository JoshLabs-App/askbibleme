...
async function memberReadingSyncViaSite(
  ...
  try {
    const res = await fetchWithTimeout(toAbsoluteUrl(base, "/api/mobile/member/reading-sync"), {
      ...init,
      headers: {
    ...
    if (__DEV__) console.warn("[memberReadingSync] site fetch failed", base, err);
