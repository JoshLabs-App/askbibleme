...
export function getShellScriptureWantPlaying(): boolean {
  return isStreamWanted(getPlaybackSnapshot().scripture);
}
 ...
 */
export function subscribeShellScriptureWantPlaying(listener: () => void): () => void {
  let last = getShellScriptureWantPlaying();
  return subscribePlaybackState(() => {
    const next = getShellScriptureWantPlaying();
    if (next === last) return;
    last = next;
    listener();
  });
