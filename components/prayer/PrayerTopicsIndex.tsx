...
function TopicLink({ catId, topicId, title }: { catId: string; topicId: string; title: string }) {
  return (
    <Link
      ...
      className="font-medium text-ink/88 underline decoration-ink/22 underline-offset-[0.22em] transition hover:text-ink hover:decoration-ink/40"
    >
      {title}
    </Link>
  );
...
> = {
  self: (_topics, catId, T) => (
    <p className="text-[15px] leading-[1.75] text-ink/82">
        ...
        return (
          <section key={cat.id} id={cat.id} className="scroll-mt-20 py-12 first:pt-14">
            <header className="max-w-prose">
              <h2 className="font-serif text-[1.22rem] font-medium tracking-tight text-ink/90">{cat.title}</h2>
              {CATEGORY_DESC[cat.id] ? <p className="mt-2 text-[14px] leading-relaxed text-ink/70">{CATEGORY_DESC[cat.id]}</p> : null}
              <p className="mt-3">
                <Link
                  href={`/prayer/${cat.id}`}
                  className="text-[13px] font-medium text-ink/75 underline decoration-ink/20 underline-offset-[0.2em] transition hover:text-ink hover:decoration-ink/40"
                >
                  进入「{cat.title}」下的全部主题 →
                </Link>
              </p>
            ...
            <div className="mt-8 max-w-prose">
              {proseFn ? (
                ...
                <p className="text-[15px] leading-[1.8] text-ink/82">
                  {cat.topics.map((t, ti) => (
                    <span key={t.id}>
                      {ti > 0 ? "、" : null}
...
export function PrayerTopicsGuideIntro() {
  return (
    <section className="max-w-prose border-b border-ink/10 pb-12">
      <p className="text-[11px] font-medium uppercase tracking-[0.14em] text-muted">如何祷告</p>
      <h2 className="mt-2 font-serif text-[1.28rem] font-medium tracking-tight text-ink/90 sm:text-[1.35rem]">让经文成为你与神的对话</h2>
      <div className="mt-6 space-y-4 text-[15px] leading-[1.82] text-ink/82">
        <p>
          祷告不只是向神说话，
          <br />
          也是用祂的话来回应祂。
        ...
        <p>
          就像主祷文所教导的（
          <Link href="/read/MAT/6" className="text-ink/85 underline decoration-ink/25 underline-offset-[0.22em]">
            马太福音 6:9–13
          </Link>
          ），
          <br />
