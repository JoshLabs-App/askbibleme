...
/** 三轨读经：章节内手动推进指针（深度读经计划不在章节页展示推进控件）。 */
export function ReadChapterReadingPlanAdvance({ bookId, chapter }: Props) {
  const { prefs } = useEffectiveReadingPlanPrefs();
  const { progress: tripleProgress, refresh: refreshTriple } = useTripleLoopProgress();
  ...
  }, [isTriple, tripleProgress]);

  const tripleTrack = isTriple ? trackForBookId(bookId) : null;
  const tripleMatches = Boolean(
    tripleTrack && pointerMatchesTrack(tripleProgress, tripleTrack, bookId, chapter),
  );

  if (!isTriple || !tripleTrack || !tripleMatches) {
    return null;
  }

  const trackTitle = tripleLoopTrackTitle(tripleTrack);

  const advance = async () => {
    setSaving(true);
    try {
      await markTodayReadingChapterVisit(bookId, chapter);
      await advanceTripleLoopProgressTrack(tripleTrack);
      refreshTriple();
    } finally {
      setSaving(false);
    }
  ...
  return (
    <View style={styles.box} accessibilityLabel={t("pages.read.tripleLoopAdvanceAria")}>
      <Text style={styles.hint}>{tFormat("pages.read.tripleLoopAdvanceHint", { track: trackTitle })}</Text>
