...
- **读后两版（陪你探索 / 查找资料）的语言**：Josh 指出「原来是有英文内容的」——`info-edition.sqlite` 里确实中英两套俱全
  （`info_edition_v1` / `info_edition_v1_en` 各 ~1190 章，`role_356f0ffb` / `role_guide_v2_en` 同）。之前原生只取中文那套、且英文面整个不出。
  现在两版常显，内容语言按 RN 的 `prefersEnglishInfoEdition`（译本是英文 → 英文那套）再加一条：界面是英文也用英文那套；
  其余语种（西语等没有对应内容）跟界面语言走中文那套。要的那套缺这一章时退回另一套。
- **开放全量后的启动开销**（Josh 问「会不会影响打开 App 的速度」）：安卓模拟器冷启动实测——没有目录 1.08–1.13 秒，
  第一版实现（启动时在主线程解析 92KB JSON + 每次 `find()` 都重拼 435 条的表）1.16–1.30 秒，多约 120 毫秒。
  改成：缓存落盘存 TSV（39KB，按 \t / \n 切，1–2 毫秒读完）、合并表与 id 索引按 `RemoteTranslations.revision` 缓存住，
  再测 1.10–1.15 秒，与没有目录时持平。网络刷新本来就在后台、一天一次，不挡界面。
- 译本面板的语言行打开时会滚到当前语言（几百本时当前语言常在最右边）。
- **语言行按使用人数排**（Josh 2026-09-10）：原来按「我们有几个版本」排，梵语 22 本会顶在最前面。
  新增 `data/language-speakers.json` + `tools/gen-language-order.mjs` → 两端 `LanguageOrder`（82 种语言的名次 + 中英名字，`check:language-order` 对拍）。
  表里没有的语言排在后面（再按版本数）。名字优先用这张内置表：目录接口老版本只给中文名，英文界面不该显示中文语言名。
- **书卷名跟版本走**（Josh：「网上把圣经内容读出来了，目录书卷应该也有对应的」）：上游 `/bibles/{id}/books` 有该版本自己的书卷名，
  但整包带上每章每节的 id（几百 KB）→ 新增网站接口 `/api/mobile/bible/youversion/books?versionId=`，只留 id / 名称 / 缩写 / 章数（几 KB），
  服务端缓存 6 小时。两端 `RemoteBookNames`：切到在线译本时取一次，落盘 Caches/book-names/<版本号>.tsv，目录页、章节选择、章标题都用它
  （西语版本 → Génesis、Éxodo、Mateo）。分组标题（摩西五经 / 四福音书）仍是界面语言，那是界面文案不是版本内容。
- **非中英版本的两处混搭一并修掉**：章标题原来会写成「Génesis 第1章」→ 这类版本用「Génesis 1」；
  段落小标题（我们只有中英两套）在这类版本下不出，段落照分。`ReadDisplayLocale.isForeign` 两端同一条判断。
- **法语 / 西语等版本下，界面小字也不留中文**（Josh 2026-09-10 看到「Bible J.N. Darby · 经文由 YouVersion 提供」「第2章」）：
  章页的署名行、上一章 / 下一章、章标题都走 `chromeLocale`（这类版本 = 英文）；读后两版整块不出
  —— 库里只有中英两套资料，没有对应语言就不展示（Josh：「没有语言就不展示更合适」）。
- 这两条要网站部署后才生效（`/books` 是新接口；目录接口的语言英文名同理）。
...
## 五、待办 / 待 Josh 决定（长期维护，别混进聊天）

- **iOS 商店版「探索/资料」两版全部打不开（2026-09-15）**
  - 现状：R2 `askbible-media/bible/info-edition.sqlite` 从未上传（404），已于 2026-09-15 补传，线上 200。iOS 下载器不查 HTTP 状态，把 404 文本存成了本地 sqlite，已点过的手机会永远显示「暂时无法加载本章讲解」；安卓查了状态码，点「重试」即恢复。已改 iOS：校验 HTTP 状态 + SQLite 文件头，启动时清掉坏文件；1.1.1(129) 于 2026-09-15 提交审核（WAITING_FOR_REVIEW，批准后自动发布）。安卓无需发版。
  - 影响：已装商店 iOS 版且点过这两版的用户，要等新版或删 App 重装才恢复。
  - 需要 Josh 决定：已决定发版（2026-09-15），剩等苹果审核。

Josh 2026-09-11：「需要我注意、你还没做的，要单独拿出来，不要混在回复里。」
这份清单就是那个出口。每条写清楚：**现状**、**影响**、**需要你决定什么**。
做完的移到最下面「已关闭」，带日期。

### 待你决定

#### 1. 在线译本没有跟读高亮
- 现状：YouVersion 接口不提供逐节时间点。今天已改成「没有时间点就不高亮」，正文照常朗读。
- 影响：429 个在线译本里，即使有朗读也不会有高亮跟读；内置的和合本、WEB 不受影响。
- 需要你决定：就这样（推荐），还是将来为常用的几个外语版本自建时间轴（要跑一次强制对齐，成本按版本数算）。

#### 2. 在线译本的朗读覆盖有限
- 现状：在线译本的音频走网站代理，只有 18 个已知版本能播，其余没有朗读键。
- 需要你决定：是否值得为更多版本找音源，还是维持现状。

#### 3. 使用时长 / 最近阅读上云没有历史
- 现状：2026-09-11 起这两项开始同步，手机上已累积的值会在下次同步推上去。
- 影响：更早在别的设备上产生的使用时长、最近阅读记录没有历史可追。
- 需要你决定：知悉即可，无需动作。

#### 4. RN 版不认识新增的两个同步键
- 现状：`appUsageTime`、`recentChapters` 是原生 + 网站新增的；RN 版（Expo）不读不写这两项。
- 影响：RN 版不会破坏数据，但也不会同步这两项。
- 需要你决定：RN 版还要不要继续维护同步；如果要，我把这两个键补进 RN。
...
#### A. 一次 2.1 GB 的内存异常
- 现状：2026-09-10 21:30 系统日志记录 AskBible 占到 2.1 GB 被内存守护杀掉。之后所有复现里内存都正常（首页 25 MB，音乐页峰值 95 MB）。
- 后续：如果再遇到「用着用着自己退」，告诉我当时在哪一页、放了多久，我按这条线索查。

### 已关闭

- 2026-09-11 **每日读经提醒**：用户菜单里的开关 + 时间（默认 07:00），两端本地通知；
  安卓用 AlarmManager 自续 + 开机重排，并申请 Android 13 的通知权限。
- 2026-09-11 **删除账户**：用户菜单底部，走既有的 `DELETE /api/mobile/auth/account`，前面有一张说明后果的确认卡片。
- 2026-09-11 **划重点**：长按 → 划重点，手指划过即上色（Josh 要的划动式，不是逐字点），四色 + 擦除 + 完成；
  存储与 RN / 网页同一份 `askbible-read-verse-text-highlights-v1`，并进会员同步的 highlights blob。
- 2026-09-14 **划重点 dispatch 优化**（commit `43936fcd`）：改同步 `MutableMap<Int,(Offset)->Unit>` 分发，消除一帧延迟；修节号处 runStart==0 高亮空洞；真机（Samsung S23 Ultra）验收通过——跨段落（第2→3节）双 Block 均高亮，节号区不漏。
- 2026-09-11 iPhone「打开就闪退」：设备侧安装状态损坏，卸载重装恢复，非代码问题。
