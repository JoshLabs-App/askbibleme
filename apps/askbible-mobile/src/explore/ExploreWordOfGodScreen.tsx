...
import { clearExploreChapterVerseCache, loadExploreVerseTextsForRefsProgressive, scheduleExploreCategoryVerseLoad } from "./load-explore-category-verses";
...
function parseWordOfGodRef(raw: string): ParsedWordOfGodRef | null {
  ...
  const bookId = getExploreModulesContent().wordOfGod.bookAbbrToId[abbr];
  ...
  return { raw, bookId, chapter, verseList, verseLabel };
}

export function ExploreWordOfGodScreen() {
  const modules = useExploreModulesBundle();
  const wordOfGodCategories: ExploreModulesCategory[] = modules.wordOfGod.categories;
  const wordOfGodTitlesEn = modules.wordOfGod.titlesEn;
  ...
  const categoryTitles = locale === "en" ? wordOfGodTitlesEn : null;

  const parsedRefByRaw = useMemo(() => {
    const entries = wordOfGodCategories.flatMap((category) => category.refs).map((raw) => [
      raw,
  ...
  useEffect(() => {
    ...
    const category = wordOfGodCategories[expandedCategoryIndex];
    if (!category) return;
    ...
    <View style={shared.root}>
      ...
      >
        ...
        <View style={styles.accordionList}>
          {wordOfGodCategories.map((category, index) => (
