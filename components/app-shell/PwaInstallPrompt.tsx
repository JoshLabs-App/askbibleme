...
function shouldSuppressPrompt(): boolean {
  ...
  if (!APP_INSTALL_IOS_URL && !APP_INSTALL_ANDROID_URL) return true;
  return false;
}
...
export function PwaInstallPrompt() {
  ...
  const scheduleShow = useCallback(
    () => {
      ...
      showTimerRef.current = window.setTimeout(() => {
        showTimerRef.current = null;
        if (immersive || shouldSuppressPrompt() || !shouldOfferOnPath(pathname)) return;
        setVisible(true);
      }, SHOW_DELAY_MS);
    },
    [clearShowTimer, immersive, pathname],
  );
  ...
  }, [clearShowTimer, hide, immersive, scheduleShow]);
