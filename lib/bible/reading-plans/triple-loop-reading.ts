...
export function normalizeTripleLoopReadingState(
  ...
  return {
    ...
    wisdom: normalizePointerInOrder(
...
export function tripleLoopChapterUnitLabel(bookId: string): "章" | "篇" {
  return bookId === "PSA" ? "篇" : "章";
}

export function formatTripleLoopReadingLine(bookId: string, chapter: number): string {
  const name = bookMeta(bookId)?.bookName || bookId;
  return `${name} ${chapter}`;
}
...
export function formatTripleLoopReadingLineVerbose(bookId: string, chapter: number): string {
  const name = bookMeta(bookId)?.bookName || bookId;
  if (bookId === "PSA") {
    return `${name} 第 ${chapter} 篇`;
  }
  return `${name} 第 ${chapter} 章`;
}
...
export function advanceTripleLoopTrack(state: TripleLoopReadingState, track: TripleLoopTrack): TripleLoopReadingState {
  ...
  return { ...state, [track]: nextPtr };
}

/** 三轨各前进一章（诗篇按篇），用于按复活节历元推算日历进度。 */
...
export function tripleLoopTrackTitle(track: TripleLoopTrack): string {
  if (track === "ot") return "旧约循环";
  if (track === "nt") return "新约循环";
  return "智慧书循环";
}
