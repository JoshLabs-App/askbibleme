#!/usr/bin/env node
/**
 * WEB 整章 MP3 + web-en 逐节正文 → stable-ts 对齐 → public/verse-timings/web-en/
 *
 *   npm run audio:web-en-timings -- --book MAT --chapter 1 [--model small] [--force]
 *   npm run audio:web-en-timings -- --all [--continue-on-error] [--delay=500]
 */
...
const MANIFEST_PATH = path.join(cwd, "data", "bible", "web-en-chapter-audio-manifest.json");
const WEB_JSON = path.join(cwd, "data", "bible", "uploads", "web-en.json");
const WHISPER_SCRIPT = path.join(__dirname, "whisper-verse-align.py");
const TIMINGS_DIR = path.join(cwd, "public", "verse-timings", "web-en");
...
function resolveWebAudioPath(localFilename) {
  const root = process.env.DATA_ROOT?.trim() || process.env.CUV_AUDIO_DATA_DIR?.trim();
  const candidates = [path.join(cwd, "public", "audio", "web-en", localFilename)];
  if (root) candidates.push(path.join(root, "audio", "web-en", localFilename));
...
function loadWebVerses(bookId, chapter) {
  if (!fs.existsSync(WEB_JSON)) {
    throw new Error(`Missing ${WEB_JSON}`);
  }
  const raw = JSON.parse(fs.readFileSync(WEB_JSON, "utf8"));
  const chMap = raw.books?.[bookId]?.[String(chapter)];
  if (!chMap || typeof chMap !== "object") {
    throw new Error(`No WEB verses for ${bookId} ${chapter}`);
  }
...
async function alignChapter(entry, model, tmpDir, force) {
  ...
  const audioFile = resolveWebAudioPath(localFilename);
  if (!audioFile) {
    throw new Error(`MP3 missing: ${localFilename} (run npm run audio:web-en-pull)`);
  }

  console.log(`  [verses] ${bookId} ${chapter} from web-en…`);
  const verses = loadWebVerses(bookId, chapter);
...
if (!fs.existsSync(MANIFEST_PATH)) {
  console.error(`Missing ${MANIFEST_PATH}. Run npm run audio:web-en-manifest`);
  process.exit(1);
}
...
if (!bookArg && !doAll) {
  console.log(`Usage:
  npm run audio:web-en-timings -- --book MAT --chapter 1 [--model small] [--force]
  npm run audio:web-en-timings -- --book MAT --all
  npm run audio:web-en-timings -- --all [--continue-on-error] [--delay=500]
`);
  process.exit(0);
}

const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "askbible-web-en-whisper-"));
let ok = 0;
let fail = 0;

try {
  console.log(`WEB timings → ${TIMINGS_DIR}`);
