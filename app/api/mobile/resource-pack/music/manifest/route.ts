...
function normalizeMusicAssetPath(raw: string | undefined): string | null {
  const s = String(raw ?? "").trim();
  if (!s.startsWith("/music/")) return null;
...
async function collectAssets(cwd: string, store: MusicCompanionStore): Promise<ManifestAsset[]> {
...
export async function GET() {
  try {
    ...
    const hash = createHash("md5");
    hash.update(JSON.stringify(store));
    hash.update("\n---assets---\n");
    ...
    const packVersion = `music-v1-${hash.digest("hex").slice(0, 16)}`;
