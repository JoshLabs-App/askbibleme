...
import { EXPLORE_CURATED_700_VERSE_KEYS } from "@/lib/explore/explore-curated-700-verse-keys";
import {
  EXPLORE_CURATED_700_POOL_SCOPE_ID,
...
export type WriteCuratedPrayerPoolResult = {
  scopeId: string;
  verseCount: number;
  chunkCount: number;
...
/**
 * 从固定 verseKey 列表生成静态祷告池（探索默认 700 句等）。
 */
export async function writeCuratedPrayerPool(
  cwd: string,
  options: {
    scopeId?: string;
    verseKeys?: readonly string[];
  } = {},
): Promise<WriteCuratedPrayerPoolResult> {
  const scopeId = options.scopeId ?? EXPLORE_CURATED_700_POOL_SCOPE_ID;
  const verseKeys = options.verseKeys ?? EXPLORE_CURATED_700_VERSE_KEYS;
  if (verseKeys.length !== EXPLORE_CURATED_700_TARGET_COUNT && scopeId === EXPLORE_CURATED_700_POOL_SCOPE_ID) {
    throw new Error(
      `writeCuratedPrayerPool expected ${EXPLORE_CURATED_700_TARGET_COUNT} keys, got ${verseKeys.length}`,
    );
  }
  ...
  for (const verseKey of verseKeys) {
    ...
    const normalizedKey = verseKeyFromVerseRef(ref);
    ...
    `${JSON.stringify(
      {
        ...
        verseKeys: resolved.map((r) => r.verseKey),
      },
      null,
      0,
    )}\n`,
