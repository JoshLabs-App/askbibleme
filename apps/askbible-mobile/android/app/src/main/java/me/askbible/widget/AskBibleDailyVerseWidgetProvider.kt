...
class AskBibleDailyVerseWidgetProvider : AppWidgetProvider() {
  ...
  companion object {
    ...
    /** 当前 snapshot 里的 verseKey 列表（供 App 跟随钉句）。 */
    fun verseKeysFromSnapshot(context: Context): List<String> {
      return loadVerses(context).map { it.verseKey.trim() }.filter { it.isNotEmpty() }
    }

    /** 今日金句：挂件快照第一句，供音乐闹钟全屏复用。 */
    fun firstSnapshotVerse(context: Context): Pair<String, String>? {
      val first = loadVerses(context).firstOrNull() ?: return null
      return first.line to first.ref
    }
    ...
    private fun applyVerseWithFade(
      views: RemoteViews,
      context: Context,
      verses: List<WidgetVerse>,
      verse: WidgetVerse,
      verseIndex: Int,
      locale: String,
      typography: WidgetTypography.Resolved,
      isSmall: Boolean,
      textScale: WidgetTypography.TextScale,
    ) {
      ...
      // RemoteViews 每次整树重建：必须把旧句绑回当前页，ViewFlipper 才能真正淡出旧文。
      val previousVerse = verses.getOrNull(lastIndex)
      if (previousVerse != null) {
        val previousTypography = WidgetTypography.resolve(previousVerse.line, isSmall, textScale)
        bindVersePanel(views, panelIds(currentChild), previousVerse, locale, previousTypography)
      } else {
        bindVersePanel(views, panelIds(currentChild), verse, locale, typography)
      }
    ...
    private fun buildRemoteViewsInternal(
      ...
      val verseIndex =
        if (verses.isNotEmpty()) {
          WidgetRotationState.currentIndexForVerses(context, verses.map { it.verseKey })
        } else {
          0
        }
      val verse =
      ...
      // 点击整卡打开 App 首页（不进读经章、不控制播放）。
      val launchIntent =
        Intent(context, MainActivity::class.java).apply {
          action = Intent.ACTION_VIEW
          data = Uri.parse("askbible://")
