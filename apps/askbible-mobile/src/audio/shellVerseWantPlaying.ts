...
export function getShellVerseWantPlaying(): boolean {
  return isStreamWanted(getPlaybackSnapshot().verse);
}
 ...
 */
export function subscribeShellVerseWantPlaying(listener: () => void): () => void {
  let last = getShellVerseWantPlaying();
  return subscribePlaybackState(() => {
    const next = getShellVerseWantPlaying();
    if (next === last) return;
    last = next;
    listener();
  });
