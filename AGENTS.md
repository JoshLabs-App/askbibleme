...
其它 AI / Agent 接手前先读：**`docs/HANDOFF.md`**（启动顺序、硬禁令、仓库地图、命令、当前状态）。
...
**改完不要说「你测一下」。** 自己跑完、自己看结果，再向 Josh 汇报做了什么、验了什么。
他手上没有 adb、没有日志，让他手测等于把最费时的一步丢回去。

播放相关的改动跑这个：

```bash
./scripts/verify-mobile-playback.sh --build     # 打包 + 装机 + 全套回归
```

十五项：音乐起播、金句叠加（含图标黄没黄）、关音乐留金句、金句自动接句、
反复点音乐金句不断、播着音乐开读经、读经自动接章、进度轴在走、锁屏暂停/播放、无崩溃。

脚本按屏幕分辨率选坐标（`wm size`），三星 S23 Ultra 和 Pixel XL 各一套：

```bash
ANDROID_DEVICE=HT74M0200477 ./scripts/verify-mobile-playback.sh --build   # Pixel XL
```

**日志正常不等于界面正确。** 2026-09-08 有两次账本完全干净、界面却是错的：读经换章后
页面没翻页、金句换句后文字冻在第一句。原因同一个——删掉重复决策时，UI 更新恰好挂在
被删的那条链上。所以脚本每个场景都会截图，**跑完必须自己打开图看**，确认屏幕上的
章号/经文与最后播放的音轨对得上，不能只看 ✓。

换了机型或改了界面布局，脚本顶部对应那一组坐标要重新量——**量错的坐标会伪造出 bug**。
2026-09-08 花了两个多小时追「iOS 上 JS 抢在原生前面换章」，最后发现是手动测试时那一下
「拖到章末」点在了下一章键上：我一直在自己按下一章，然后把它读成 bug。
分辨这件事的办法是**让它自己跑完**——一次不碰屏幕的完整播放，那次 iOS 老老实实
`nativeAdvanced scripture GEN-4`。

### 账本要能说出「是谁发的命令」

原生账本里每条播放都长一样（`play scripture GEN-2`），看不出来自用户点击、章末续播还是
JS 兜底。所以每条播放命令带一个 `origin` 标签，两端桥接处打进日志：

```
js userPlay scripture origin=chapter-kick
play scripture …/GEN-2.mp3 q=0 -> audible=[SCRIPTURE]
```

标签**只进日志，不进状态机**——状态机不该知道谁调用了它。需要更细的来源时，用
`withPlaybackOrigin("名字", () => …)` 把那一段调用圈起来，别再加临时 console.warn：
加日志、重新打包、复现、读栈、删日志这一轮，一次就是半小时起。
