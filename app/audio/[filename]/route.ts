...
async function serveFile(
  ...
  if (!isSafeCuvChapterAudioFilename(decoded)) {
    return new Response("Not found", { status: 404 });
  }

  const filePath = resolveCuvChapterAudioFilePath(decoded);
  if (!filePath) {
    return new Response("Not found", { status: 404 });
  }
...
export async function HEAD(req: NextRequest, ctx: RouteCtx): Promise<Response> {
  const res = await serveFile(req, (await ctx.params).filename);
