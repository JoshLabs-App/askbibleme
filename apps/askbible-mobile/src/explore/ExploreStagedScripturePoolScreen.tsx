...
import { localizeZhText, resolveUiText, t } from "../i18n/site-copy";
...
import {
  ...
  scheduleExploreCategoryVerseLoad,
} from "./load-explore-category-verses";
import type { ExploreModulesCategory, ExploreModulesRemoteStagedModule } from "./exploreModulesBundleCore";
...
function parseRef(raw: string, bookAbbrToId: Record<string, string>): ParsedRef | null {
  ...
  const bookId = bookAbbrToId[abbr];
...
function resolveRemoteModulePageTitle(
  locale: AppLocale,
  module: ExploreModulesRemoteStagedModule,
  entryLabel: string,
): string {
  if (locale === "en") {
    return (module.pageTitleEn || module.pageTitle || entryLabel).trim();
  }
  const raw = locale === "zh-TW"
    ? (module.pageTitleTw || module.pageTitle || entryLabel).trim()
    : (module.pageTitle || entryLabel).trim();
  return locale === "zh-TW" ? localizeZhText(locale, raw) : raw;
}
...
/** 预埋新经文池：正文来自 exploreHome.remoteModules，结构与窄门/敬拜等汇编一致 */
export function ExploreStagedScripturePoolScreen({ entryId, module }: Props) {
  const bundle = useExploreModulesBundle();
  const stagedEntry = getExploreStagedEntry(entryId);
  const categories: ExploreModulesCategory[] = module.categories ?? [];
  const bookAbbrToId = module.bookAbbrToId ?? {};
  const categoryTitlesEn = module.titlesEn;
  ...
  const entryLabel = stagedEntry
    ? resolveExploreStagedEntryLabel(stagedEntry, locale, bundle)
    : entryId;
  ...
  const parsedRefByRaw = useMemo(() => {
    const entries = categories.flatMap((category) => category.refs).map((raw) => [
      raw,
      parseRef(raw, bookAbbrToId),
    ] as const);
    return Object.fromEntries(entries) as Record<string, ParsedRef | null>;
  }, [bookAbbrToId, categories]);
  ...
  useEffect(() => {
    ...
    const category = categories[expandedCategoryIndex];
    if (!category) return;
