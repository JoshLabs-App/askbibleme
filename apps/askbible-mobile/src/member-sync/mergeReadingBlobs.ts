...
function mergeStringSetRecords(
  a: unknown,
  b: unknown,
  field: "completed" | "doneKeys" | "completedDates",
): unknown {
  ...
  const version = (v: unknown): number => {
    ...
    return (v as { version?: unknown }).version === 1 ? 1 : 1;
  };
...
function mergeBlobPair(
  ...
  return {
    ...
    value: mergeBlobValue(key, older.value, newer.value),
  };
}
