...
- **读后两版（陪你探索 / 查找资料）改跟界面语言**：讲解 / 发现两版只有中文内容，之前跟译本语言（displayLocale）出，
  于是读西语版本时底下会冒出中文两版、读英文版本时中文用户又看不到。现在按界面语言：中文界面照出（读任何版本都能看中文讲解），
  英文或其它语言界面不出 —— 没有对应语言的内容就不拿中文顶（Josh 2026-09-10）。
